...
public class AnimalDAO {
	...
	public boolean incluirAnimal(Animal animal) {
		try {
			...
			statement.executeUpdate(
					"INSERT INTO animal(nome, descricao, sexo, datanascimento, cidade, valordoacao, imagem, tipo_id, usuario_id) VALUES(" + values + ");");
			conn.close();
	...
	public boolean alterarAnimal(Animal animal) {
		try {
			...
			if(statement.executeUpdate("UPDATE usuario SET " + set + " WHERE id = " + animal.getId() + ";") != 0) {
				return true;
			}
